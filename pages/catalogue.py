...
if selected:
    queryFlower = select(Flower.name, Flower.short_desc, Flower.image_link).where(
        Flower.name.in_(selected)
    )
    queryBouquet = select(Bouquet.name, Bouquet.meaning, Bouquet.image_link).where(
        Bouquet.name.in_(selected)
    )
else:
...
with bouquets:
    for m in range(0, len(topBouquets), 3):
        ...
        for n, (bouquet, desc, image_path) in enumerate(batch):
            with cols[n]:
                st.image(image_path, use_container_width=True)
                st.subheader(bouquet, anchor=False)
                st.write(f"{desc}")
                if st.button(
                    f"View",
                    key=f"view_button_bouquet_{m+n}",
                    use_container_width=True,
                    type="primary",
                ):
